...
commands.each do |on, x_dim, y_dim, z_dim|
    if x_dim.to_a.intersect?(INITIALIZATION_REGION) && y_dim.to_a.intersect?(INITIALIZATION_REGION) && z_dim.to_a.intersect?(INITIALIZATION_REGION)
        x_dim.to_a.product(y_dim.to_a, z_dim.to_a).each do |x, y, z|
            matrix[x][y][z] = on
        end
    end
end

number_of_cubes_turned_on = matrix.map{|x_index, y| y.map{|y_index, z| z.values}}.flatten.count{|location| location == true}
